...
export async function POST(req: Request) {
  try {
    ...
    if (!email || !email.includes("@")) return json(400, { error: "Ugyldig e-post" });
    if (!password || password.length < 6) return json(400, { error: "Passord må være minst 6 tegn" });
    if (!productCode) return json(400, { error: "Produktkode mangler" });
    ...
    const { data: pkg, error: pkgErr } = await admin
      ...
      .select("id, product_code, home_id, claimed_at")
      .eq("product_code", productCode)
      .maybeSingle();

    if (pkgErr) return json(500, { error: pkgErr.message });
    if (!pkg) return json(400, { error: "Ugyldig produktkode (finnes ikke)" });
    if (pkg.home_id) return json(400, { error: "Produktkode er allerede i bruk" });

    // 2) Opprett bruker (MVP: email_confirm=true for å slippe e-post-rate)
    ...
    if (!userId) return json(500, { error: "Kunne ikke opprette bruker" });

    // 3) Opprett home + membership(admin) + contact_methods
    ...
    const { error: claimErr } = await admin
      ...
      .eq("id", pkg.id)
      .is("home_id", null); // ekstra sikkerhet mot race

    if (claimErr) return json(500, { error: `product_packages claim: ${claimErr.message}` });
